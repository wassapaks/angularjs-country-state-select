 ...
 */
angular.module('ngCountryStateSelect', [])
    .factory('src', function(){
        return{
...
function countryDirective(src) {
    ...
    return {
        ...
        template: [
            ...
            'required ',
            'title= "select country in drop-down-list"> ',
...
function stateDirective(src,$timeout) {
    function stateController() {
        ...
        self.init = function(country) {
            var countries = src.countries;
            var index = 0;
            var x = 0;
            ...
            var states = src.states;

            var state_arr = states[index+1].split("|");
    ...
    return {
        ...
        template: [
            ...
            'required ',
            'title= "Select state in drop-down-list"> ',
            '<option value="">Select state</value> ',
            '</select>'
        ].join(''),
        link: function(scope, element, attrs, ctrl) {
            ctrl.init(scope.country);
